...
export function Navbar({ isDarkMode, toggleTheme }: NavbarProps) {
  ...
  return (
    <nav className="sticky top-0 w-full flex justify-between items-center mb-6 px-6 md:px-12 py-4 z-50 shrink-0 border-b border-gray-300 dark:border-gray-700 backdrop-blur-xl bg-background/80 transition-all duration-300">
      {/* Project Name */}
        ...
        {user ? (
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 p-1 pr-3 rounded-full hover:bg-gray-100 dark:hover:bg-zinc-800 transition-all border border-transparent hover:border-gray-200 dark:hover:border-zinc-700">
              {user.image ||
              user.avatarUrl ||
              (user.photos && user.photos[0]) ? (
                <img
                  src={user.image || user.avatarUrl || user.photos?.[0]?.value}
              ...
              {isProfileOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-64 p-4 z-50 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-zinc-700/50">
